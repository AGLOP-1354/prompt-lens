  ...
  return (
    <div className="h-screen w-screen overflow-hidden bg-slate-50 flex flex-col md:flex-row">
      <motion.div
        ...
        className="w-full md:w-1/2 h-full bg-white relative border-r border-slate-200 overflow-auto"
      >
        <div className="h-full p-4 md:p-8 flex flex-col min-h-[400px] pb-24 md:pb-8">
          <Textarea
            value={prompt}
            onChange={(e: ChangeEvent<HTMLTextAreaElement>) => setPrompt(e.target.value)}
            placeholder="프롬프트를 입력하세요..."
            className={cn(
              "flex-1 resize-none text-base md:text-lg border-0 focus-visible:ring-0 focus-visible:ring-offset-0 shadow-none p-0",
          ...
          <motion.div
            ...
            className="mt-4 flex flex-col md:flex-row md:items-center md:justify-between gap-2"
          >
            <div className={cn(
            ...
            <AnimatePresence>
              {isInvalid && (
                <motion.span
                  ...
                  animate={{ opacity: 1, x: 0 }}
                  exit={{ opacity: 0, x: -10 }}
                  className="text-red-600 text-xs md:text-sm flex items-center gap-1"
                >
              ...
              {isOverLimit && (
                <motion.span
                  ...
                  animate={{ opacity: 1, x: 0 }}
                  exit={{ opacity: 0, x: -10 }}
                  className="text-orange-600 text-xs md:text-sm flex items-center gap-1"
                >
                  <AlertCircle className="w-3 h-3 md:w-4 md:h-4" />
          ...
          <p className="mt-1 md:mt-2 text-xs text-slate-400">
        ...
        <motion.button
          ...
          className="fixed md:absolute bottom-4 md:bottom-8 left-4 md:left-1/2 right-20 md:right-auto md:-translate-x-1/2 px-6 md:px-7 py-3 md:py-3.5 bg-blue-600 hover:bg-blue-700 text-white rounded-full shadow-lg transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100 flex items-center justify-center gap-2 md:gap-2.5 font-semibold text-sm md:text-base z-10"
        >
          {isLoading ? (
            <>
      ...
      <motion.div
        ...
        className="w-full md:w-1/2 h-full bg-gradient-to-br from-blue-50 via-white to-indigo-50 overflow-auto"
      >
        <div className="h-full p-4 md:p-8 pb-32 md:pb-8">
          {isLoading ? (
            <div className="flex flex-col items-center justify-center h-full min-h-[400px] py-8">
              ...
              >
                <motion.div
                  className="w-20 h-20 md:w-24 md:h-24 rounded-full border-4 border-blue-200"
                  animate={{ rotate: 360 }}
                  transition={{ duration: 2, repeat: Infinity, ease: 'linear' }}
                />
                <motion.div
                  className="absolute top-0 left-0 w-20 h-20 md:w-24 md:h-24 rounded-full border-t-4 border-blue-600"
                  animate={{ rotate: 360 }}
              ...
              <motion.div
                className="mt-6 md:mt-8 space-y-2 w-full max-w-md px-4"
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
              ...
              >
                {['명확성 평가', '구체성 분석', '구조 검토', '완전성 확인', '효율성 측정'].map((step, i) => (
                  <motion.div
                    key={step}
                    className="flex items-center gap-3 px-3 md:px-4 py-2 bg-white/50 rounded-lg"
                  ...
                  >
                    ...
                    />
                    <span className="text-xs md:text-sm text-slate-600">{step}</span>
                  </motion.div>
                ))}
              </motion.div>
            </div>
            ...
            result.overall_score === -404 ? (
              ...
              >
                  <div className="bg-gradient-to-br from-red-50 via-orange-50 to-red-50 rounded-2xl border-2 border-red-300 p-6 md:p-10">
                    <div className="flex items-center justify-center mb-6">
                      <motion.div
                        initial={{ scale: 0, rotate: -180 }}
                        animate={{ scale: 1, rotate: 0 }}
                        transition={{ delay: 0.3, type: 'spring', duration: 0.8 }}
                        className="bg-red-100 rounded-full p-4 md:p-6"
                      >
                        <XCircle className="w-16 h-16 md:w-20 md:h-20 text-red-600" />
                      </motion.div>
                    </div>

                    <h2 className="text-2xl md:text-3xl font-bold text-red-900 text-center mb-4">
                      분석할 수 없는 프롬프트입니다
                    </h2>

                    <div className="bg-white/80 rounded-xl p-4 md:p-6 mb-6 border border-red-200">
                      <h3 className="text-base md:text-lg font-semibold text-red-800 mb-3 flex items-center gap-2">
                    ...
                    <div className="space-y-3 text-red-800">
                      <p className="text-sm md:text-base font-semibold">올바른 프롬프트 작성 가이드:</p>
                      <ul className="space-y-2 ml-2 md:ml-4">
                        <li className="flex items-start gap-2">
                          <span className="text-red-600 mt-1">•</span>
                          <span className="text-sm md:text-base">AI에게 요청할 명확한 작업이나 질문을 작성하세요</span>
                        </li>
                        <li className="flex items-start gap-2">
                          <span className="text-red-600 mt-1">•</span>
                          <span className="text-sm md:text-base">의미 있는 단어와 문장으로 구성하세요</span>
                        </li>
                        <li className="flex items-start gap-2">
                          <span className="text-red-600 mt-1">•</span>
                          <span className="text-sm md:text-base">욕설, 혐오 표현, 부적절한 내용을 포함하지 마세요</span>
                        </li>
                        <li className="flex items-start gap-2">
                          <span className="text-red-600 mt-1">•</span>
                          <span className="text-sm md:text-base">구체적인 맥락과 목적을 포함하세요</span>
                        </li>
                      </ul>
                    </div>
                  </div>
              ...
              >
                <motion.div
                  ...
                  className="bg-white rounded-xl shadow border border-slate-200 p-4 md:p-6"
                >
                <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-4 gap-4">
                  <div className="flex items-center gap-3 md:gap-4">
                    <motion.div
                      ...
                      className="text-4xl md:text-5xl font-bold bg-gradient-to-r from-blue-600 to-indigo-600 text-transparent bg-clip-text"
                    >
                      {result.overall_score}
                    </motion.div>
                    <div>
                      <div className="text-xs md:text-sm text-slate-500">종합 점수</div>
                      <div className="text-base md:text-lg font-semibold text-slate-700">{result.grade}</div>
                    </div>
                  </div>
                  <motion.button
                    ...
                    className="inline-flex items-center justify-center gap-2 px-3 py-2 rounded-lg border border-slate-300 bg-white hover:bg-slate-50 text-slate-700 text-xs md:text-sm font-medium touch-manipulation"
                  >
                  ...
                  {(Object.keys(result.scores) as Array<keyof AnalysisResult['scores']>).map((key, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="text-sm md:text-lg font-bold text-slate-900">{value}<span className="text-[10px] md:text-xs text-slate-500">/{maxScore}</span></div>
                        <div className="w-full bg-slate-200 rounded-full h-1 md:h-1.5 mt-1 md:mt-2">
                          <motion.div
                            initial={{ width: 0 }}
                            animate={{ width: `${percentage}%` }}
                            transition={{ delay: 0.5 + index * 0.05, duration: 0.6 }}
                            className="bg-gradient-to-r from-blue-600 to-indigo-600 h-1 md:h-1.5 rounded-full"
                          />
                        </div>
                      </motion.div>
                    )
              ...
              {result.summary && (
                ...
                >
                  ...
                  <p className="text-sm md:text-base text-slate-700 leading-relaxed">{result.summary.overall_assessment}</p>
                </motion.div>
              )}
              ...
              {result.summary && (!result.summary.priority_improvements || result.summary.priority_improvements.length === 0) &&
               ...
               !result.improved_prompt && (
                <motion.div
                  ...
                  className="bg-gradient-to-br from-amber-50 via-yellow-50 to-amber-50 rounded-xl border-2 border-amber-300 p-6 md:p-8"
                >
                  <div className="flex items-center justify-center mb-4">
                    ...
                    >
                      <Trophy className="w-12 h-12 md:w-16 md:h-16 text-amber-500" />
                    </motion.div>
                  </div>
                  <h3 className="text-xl md:text-2xl font-bold text-amber-900 text-center mb-3">
                    완벽한 프롬프트입니다! 🎉
                  </h3>
                  <p className="text-sm md:text-base text-amber-800 text-center leading-relaxed">
                    이 프롬프트는 이미 매우 훌륭하게 작성되어 있어 추가적인 개선이 필요하지 않습니다.<br className="hidden md:block" />
              ...
              {result.summary && result.summary.action_items && result.summary.action_items.length > 0 && (
                <motion.div
                  ...
                  className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl border border-blue-200 p-4 md:p-6"
                >
                  <h3 className="text-base md:text-lg font-bold text-slate-800 mb-4 flex items-center gap-2">
              ...
              {result.improved_prompt && (
                ...
                >
                        ...
                        <AnimatePresence mode="wait">
                          {isCopied ? (
                            ...
                            <motion.div
                              ...
                              exit={{ scale: 0 }}
                            >
                              <Copy className="w-4 h-4 md:w-5 md:h-5 text-slate-600" />
                            </motion.div>
                          )}
                        </AnimatePresence>
                      </motion.button>
                  ...
                  <div className="bg-slate-50 rounded-lg p-3 md:p-4 mb-4 border border-slate-200">
                    <p className="text-sm md:text-base text-slate-700 leading-relaxed whitespace-pre-wrap">{result.improved_prompt.text}</p>
                  </div>

                  <div className="space-y-2">
                    <h4 className="text-xs md:text-sm font-semibold text-slate-700">주요 변경사항:</h4>
            ...
            <div className="flex flex-col items-center justify-center h-full min-h-[400px] py-8">
              ...
              >
                <div className="w-24 h-24 md:w-32 md:h-32 bg-gradient-to-br from-blue-100 to-indigo-100 rounded-full flex items-center justify-center mb-6 mx-auto">
